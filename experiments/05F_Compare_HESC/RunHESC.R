...
load("/home/julian/Uni/MasterThesis/data/hesc_sce.rda")
...
for (i in 1:n_seeds) {
  ...
  psuper <- psupertime::psupertime(hesc_sce,
                                   hesc_sce$esc_day,
                                   smooth=FALSE,
                                   min_expression = 0.01,
                                   scale=TRUE,
...
cat(sprintf("HESC SPARSE LATEXROW: $%s \\pm %s$  &  $%s \\pm %s$ \n", mean_acc, std_acc, mean_sparsity, std_sparsity))
...
mean_sparsity  <- round((1 - (mean(dof) / n_input_genes)) * 100, 2)
std_sparsity <- round((sd(dof) / n_input_genes) * 100, 1)
cat(sprintf("HESC BEST LATEXROW: $%s \\pm %s$  &  $%s \\pm %s$ \n", mean_acc, std_acc, mean_sparsity, std_sparsity))
